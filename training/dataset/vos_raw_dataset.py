...
class PNGRawDataset(VOSRawDataset):
    def __init__(
        ...
        self.multitask = len(gt_folder) > 1
        ...
        if file_list_txt is not None:
            ...
            subset = os.listdir(self.gt_folder[0])  
        ...
        if self.single_object_mode:
            ...
            self.video_names = sorted(
                [
                    ...
                    for obj in os.listdir(os.path.join(self.gt_folder[0], video_name))
                ]
            )
    ...
    def get_video(self, idx):
        ...
        video_mask_roots = [os.path.join(gt_folder, video_name) for gt_folder in self.gt_folder]

        if self.is_palette:
            segment_loader = [PalettisedPNGSegmentLoader(video_mask_root) for video_mask_root in video_mask_roots]
        else:
            segment_loader = [MultiplePNGSegmentLoader(
                video_mask_root, self.single_object_mode
            )  for video_mask_root in video_mask_roots]
