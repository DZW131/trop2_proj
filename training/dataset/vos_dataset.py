...
class VOSDataset(VisionDataset):
    ...
    def construct(self, video, sampled_frms_and_objs, segment_loaders):
        """
        ...
        for frame_idx, frame in enumerate(sampled_frames):
            ...
            if isinstance(segment_loaders[0], JSONSegmentLoader):
                segments = [
                    segment_loader.load(frame.frame_idx, obj_ids=sampled_object_ids)
                    for segment_loader in segment_loaders
                ]
            else:
                segments = [
                    segment_loader.load(frame.frame_idx)
                    for segment_loader in segment_loaders
                ]
            for obj_id in sampled_object_ids:
                # Extract the segment
                if obj_id in segments[0]:
                    assert (
                        segments[0][obj_id] is not None
                    ), "None targets are not supported"
                    # segment is uint8 and remains uint8 throughout the transforms
                    segment = torch.cat(
                        [
                            segments[i][obj_id].unsqueeze(0)
                    ...
                    if not self.always_target:
                        continue
                    segment = torch.zeros(len(segment_loaders), h, w, dtype=torch.uint8)
