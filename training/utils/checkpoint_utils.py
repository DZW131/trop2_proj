...
def load_state_dict_into_model(
    ...
    remapped_state_dict = {}
    for k, v in state_dict.items():
        if k.startswith("sam_mask_decoder2"):
            remapped_state_dict[
                k.replace("sam_mask_decoder2", "sam_mask_decoders.1", 1)
            ] = v
        elif k.startswith("sam_mask_decoder."):
            remapped_state_dict[
                k.replace("sam_mask_decoder.", "sam_mask_decoders.0.", 1)
            ] = v
        else:
            remapped_state_dict[k] = v

    has_decoder1 = any(
        k.startswith("sam_mask_decoders.1.") for k in remapped_state_dict
    )
    if not has_decoder1:
        decoder1_state_dict = {
            k.replace("sam_mask_decoders.0.", "sam_mask_decoders.1.", 1): v
            for k, v in remapped_state_dict.items()
            if k.startswith("sam_mask_decoders.0.")
        }
        remapped_state_dict.update(decoder1_state_dict)
    ...
    for k, v in remapped_state_dict.items():
        if k in model_state_dict and tuple(v.shape) != tuple(model_state_dict[k].shape):
            mismatched_keys.append((k, tuple(v.shape), tuple(model_state_dict[k].shape)))
            continue
    ...
    missing_keys = list(
        dict.fromkeys(list(missing_keys) + [k for k, _, _ in mismatched_keys])
    )
